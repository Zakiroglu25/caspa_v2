...
class NotificationCubit extends Cubit<NotificationState> {
  ...
  void fetch({bool? loading,@required BuildContext? context}) async {
   ...
   String token = '';
      final result = await NotificationProvider.fetchNotificationData(
          token: token);
  ...
  Future<bool> removeNotificion({int? notificationId, bool? loading,required BuildContext? context}) async {
    if (loading ?? true) {
      emit(NotificationInProgress());
    }
